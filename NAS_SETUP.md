...
On 24 September, the ESP32-C3 connected to the nearby FRITZ!Repeater with the normal `wifi_message` firmware after reducing ESP Wi-Fi transmit power to 8.5 dBm. Default transmit power failed again in the comparison test. Three runtime restarts at 8.5 dBm reached DHCP, initialized I2S, and successfully polled the NAS over verified HTTPS with the device token. The repeater used WPA2, 2.4 GHz Wi-Fi 4, and allowed new devices. The exact electrical/RF cause is not established; this is a tested workaround for this assembled Super Mini, not a universal router requirement. Long-distance reception and exhibition-day stability still need testing. A live NAS model request returned `I AM HERE`. The initial Wi-Fi audio routine logged playback but was not audible; the USB `HI` test was audible. Wi-Fi playback now uses the same continuously buffered lookup-table synthesis as USB playback. The public play endpoint reported `deviceOnline: true`; the ESP logged `START I AM HERE` and `DONE`, and the user confirmed audible output. Phone decoding with this build still needs checking. GitHub Pages, Funnel, the NAS API and live GPT-6 Luna replies were verified previously.

### Acoustic timing trial (24 September)

The ESP firmware keeps at least 220 ms of silence between letters (previously 90 ms), uses 24 ms fades, and leaves one second of trailing silence (previously three). Frequencies and amplitude (1200) remain unchanged. The web decoder now also uses letter spacing to resolve ambiguous low tones. Existing NAS replies of at most 12 characters fit the advertised duration after shortening the tail. Longer manually supplied messages do not have this timing guarantee. The Python audio generator still uses legacy timing.

A deterministic two-reflection simulation lost letters from `STAY CALM` with the old gaps. The new gaps decode `STAY CALM`, `I AM HERE`, `I HEAR YOU`, and `HELLO` at both 44.1 and 48 kHz. These are simulated results; actual phone reception at distance still requires a comparison in the room. This softens tone edges but does not change the electronic character of the codebook.

## 1. Publish the matching phone page

GitHub Pages for `JR-4B3/ALIVE` is set to the `main` branch's `/docs` folder, at <https://jr-4b3.github.io/ALIVE/>. Include the built `docs/` files when publishing. The printable [exhibition QR](docs/ALIVE-QR.svg) points to the visitor page. The separate [debug page](https://jr-4b3.github.io/ALIVE/debug/) has microphone diagnostics, connection settings and recording controls. The NAS address is prefilled; neither page includes a private token. The debug page prepares a local WAV download first. It uploads only when the operator token is entered, and keeps the local link if the upload fails.
