...
The GitHub Pages QR page, public Funnel URL, NAS container, and a live GPT-6 Luna reply have since been verified. The ESP32 joined an open PC access point and a deliberately configured 2.4 GHz WPA2 PC access point, including when that access point used the saved iPhone hotspot password. The PC could also join and ping the ESP32's own test access point. Its radio and saved password therefore work in these controlled tests. The ESP32 still reports authentication expiry with the iPhone hotspot and home router; their exact compatibility or RF problem remains unresolved. **Play signal** correctly reports `ESP32 is offline`. On 24 September the board was left with the *silent* `wifi_diagnostic` firmware, which does not initialize I2S or play audio. Test the intended exhibition access point, then flash `wifi_message` and verify end-to-end operation before the exhibition.
