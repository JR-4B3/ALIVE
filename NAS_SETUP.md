...
The ESP firmware now keeps at least 220 ms of silence between letters (previously 90 ms), uses 24 ms fades, and leaves one second of trailing silence (previously three). Frequencies, amplitude (1200), and the deployed web decoder remain unchanged. This is compatible with existing NAS replies of at most 12 characters: the extra gaps plus the 250 ms lead still fit the old advertised duration after shortening the tail. Longer manually supplied messages do not have this timing guarantee. The Python audio generator still uses legacy timing.
